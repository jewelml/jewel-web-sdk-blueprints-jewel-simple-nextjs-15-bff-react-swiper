...
async function fetchModelData(models: string[], itemId: string): Promise<ModelResult[]> {
  try {
    ...
    const bffUrl = `http://localhost:3004/api/recommendations?item_id=${encodeURIComponent(itemId)}&models=${encodeURIComponent(models.join(','))}`;
    
    const response = await fetch(bffUrl);
    
    if (!response.ok) {
      throw new Error(`BFF Server error! status: ${response.status}`);
    }
    
    const bffResponse = await response.json();
    
    if (!bffResponse.success) {
      throw new Error(bffResponse.error || 'BFF Server returned error');
    }
    
    return bffResponse.results;
  } catch (error) {
    console.error('SSR Error:', error instanceof Error ? error.message : 'Unknown error');
    throw error;
  }
}
